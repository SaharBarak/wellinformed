...
- [ ] **SESS-04**: Daemon tick includes a session-sources run (configurable interval, default 5 min) so in-progress sessions are captured in near-realtime without user action.
- [ ] **SESS-05**: New CLI command `wellinformed recent-sessions [--hours N] [--project PATH] [--json]` prints a structured summary of recent sessions: duration, tool-call count, file list, final assistant message, git branch.
- [ ] **SESS-06**: New MCP tool `recent_sessions(hours?, project?, limit?)` — Claude queries previous session state from any new session. Returns the same structured summary as the CLI for machine consumption.
- [ ] **SESS-07**: The existing wellinformed PreToolUse hook is extended to surface a one-paragraph "previous session summary" on SessionStart, injected into Claude's context automatically. Triggers when the current session has < 3 user messages and a previous session exists within the last N hours (default 24).
